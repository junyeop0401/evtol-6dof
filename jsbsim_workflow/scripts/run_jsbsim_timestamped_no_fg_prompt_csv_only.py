...
def discover_runscripts(aircraft: str | None = None) -> list[Path]:
    paths = [
        ...
        if path.is_file()
        and path.name != "nonrotating_earth.xml"
        and "__pycache__" not in path.parts
        and "initial_condition" not in path.parts
    ]
    if aircraft:
        paths = [path for path in paths if script_aircraft(path) == aircraft]
    return sorted(paths, key=script_sort_key)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run JSBSim with selected aircraft/init/runscript and timestamp CSV outputs only.")
...
def main() -> None:
    ...
    convert_raw_to_si(raw_path, si_path)
    ...
    if skipped_sixdof_properties:
        ...
        for prop in skipped_sixdof_properties:
            print(f"  {prop}")
    print("Plotting: disabled (CSV-only runner)")
    print("Detailed ploting: skipped")
    print("Workflow Excel update: skipped")
