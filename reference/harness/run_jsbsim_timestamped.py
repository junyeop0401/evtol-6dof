...
def parse_args() -> argparse.Namespace:
    ...
    parser.set_defaults(flightgear=None)
...
def choose_flightgear_stream(requested: bool | None) -> bool:
    ...
    print()
    print("FlightGear streaming runs JSBSim with --realtime and --logdirectivefile.")
    while True:
        answer = input("Enable FlightGear visualization stream? [y/N]: ").strip().lower()
        if answer in {"", "n", "no"}:
            return False
        if answer in {"y", "yes"}:
            return True
